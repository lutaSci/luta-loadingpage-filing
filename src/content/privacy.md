...
为了判断广告是否真正带来页面到达和下载入口点击，并帮助广告平台优化投放，我们会在获得您的明确同意后启用广告效果衡量：

- Meta Pixel：向 Meta Platforms, Inc. 发送页面到达（PageView）和您明确点击下载按钮（WebsiteDownloadCtaClicked）的信号；
- Google Analytics / Google Ads：网站使用受限的手工页面与下载按钮事件。只有在您同意广告效果衡量后，Google 的广告存储（ad_storage）才会启用；广告用户数据和广告个性化默认保持关闭；
- 汝塔自有分析：我们使用显式事件了解官网和安装选择流程，不启用自动采集、会话录屏或完整网址查询参数采集。

广告平台可能接收当前安全页面网址、平台自身附加的广告点击标识（例如 fbclid、gclid、gbraid 或 wbraid 中的一个）、页面路径、入口类型、目标市场、设备/浏览器和网络信息、Cookie 或平台生成的测量标识，以及上述页面到达/下载按钮事件。Smart Link 的签名 state 会在任何第三方广告测量代码加载前从浏览器地址栏移除；我们不会向这些平台发送汝塔内部的 link_id、click_id、用户账号标识、您输入的内容或 Smart Link state。

您可以选择“仅必要”，此时 Meta Pixel 不会加载，Google 广告存储保持关闭，官网浏览、下载和汝塔自身的确定性 Smart Link 归因仍可正常工作。您可随时通过官网页脚或本隐私政策页的“广告测量设置”修改或撤回选择；撤回后页面会重新加载，以停止后续广告测量事件。第三方接收方将按照其政策处理已收到的数据：Meta 隐私政策 [https://www.facebook.com/privacy/policy/](https://www.facebook.com/privacy/policy/) ，Google 隐私政策 [https://policies.google.com/privacy](https://policies.google.com/privacy) 。

隐私声明链接：[https://lutaai.com/privacy](https://lutaai.com/privacy)
...
上述广告效果衡量条款于 2026 年 8 月 20 日更新；本政策的其他条款继续按其原生效时间适用。
